...
export function TrustStrip() {
  ...
  return <section aria-label="Store benefits" className="border-y border-border bg-surface"><div className="section-shell no-scrollbar flex snap-x gap-3 overflow-x-auto py-5 lg:grid lg:grid-cols-4">{items.map(([Icon, title, text]) => <div key={title} className="flex min-w-[78vw] snap-center gap-3 rounded-md border border-border bg-card px-4 py-4 sm:min-w-[310px] lg:min-w-0"><span className="grid size-9 shrink-0 place-items-center rounded-md bg-primary-soft"><Icon className="size-4.5 text-primary" /></span><div><h2 className="text-sm font-bold text-foreground">{title}</h2><p className="mt-1 text-xs leading-5 text-silver-muted">{title === "Delivery in Ghana" ? "Convenient delivery across supported locations." : text}</p></div></div>)}</div></section>;
}

export function ProductSections() {
  return <>
    <section id="featured" className="section-shell py-16 sm:py-24"><div className="section-heading"><div><p className="eyebrow">Demo collection</p><h2>Featured Products</h2><p>Every item shown is sample data for storefront development.</p></div><Link to="/shop" className="section-link">View all products</Link></div><div className="mt-8 grid grid-cols-1 gap-4 min-[480px]:grid-cols-2 lg:grid-cols-4">{products.filter((p) => p.featured).map((product) => <ProductCard key={product.id} product={product} />)}</div></section>
