...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/90 backdrop-blur-xl">
      <div className="border-b border-border/60 bg-surface text-foreground">
        <div className="mx-auto flex h-8 max-w-7xl items-center justify-center px-4 text-center text-[11px] font-semibold sm:justify-between sm:px-6 lg:px-8">
          ...
          <span className="hidden text-muted-foreground sm:inline">Demo storefront · Ghana-focused delivery experience</span>
        </div>
      </div>
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
         <a href="#top" className="flex h-12 w-[116px] shrink-0 items-center sm:w-[134px]" aria-label="GIMATech home">
           <img src={logoAsset.url} alt="GIMA Technologies" className="h-11 w-full object-contain object-left" />
        </a>
        <nav className="ml-3 hidden items-center gap-1 lg:flex" aria-label="Primary navigation">
          {navItems.map((item) => <a key={item} href={item === "Home" ? "#top" : `#${item.toLowerCase()}`} className="rounded-md px-3 py-2 text-sm font-semibold text-muted-foreground transition-colors hover:bg-primary-soft hover:text-primary">{item}</a>)}
        </nav>
        <div className="relative ml-auto hidden w-full max-w-sm md:block">
          <Search className="pointer-events-none absolute left-3 top-2.5 size-4 text-muted-foreground" />
           <Input value={query} onChange={(event) => setQuery(event.target.value)} className="h-10 border-border bg-surface/80 pl-9 pr-4 shadow-none focus-visible:border-primary" placeholder="Search products, brands, categories…" aria-label="Search products" />
        ...
        <div className="hidden items-center gap-1 sm:flex">
          ...
          <Button variant="ghost" size="icon" aria-label="Wishlist"><Heart /></Button>
           <Button size="icon" aria-label="Cart"><ShoppingBag /></Button>
        </div>
       ...
       {menuOpen && <div className="border-t border-border bg-background px-4 pb-5 pt-4 shadow-2xl lg:hidden">
        <div className="relative md:hidden"><Search className="pointer-events-none absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} className="h-10 bg-muted/70 pl-9" placeholder="Search tech…" /></div>
         <nav className="mt-3 grid grid-cols-2 gap-1">{navItems.map((item) => <a key={item} href={item === "Home" ? "#top" : `#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)} className="rounded-md px-3 py-2.5 text-sm font-semibold hover:bg-primary-soft hover:text-primary">{item}</a>)}</nav>
