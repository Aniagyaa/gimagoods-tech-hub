...
export function ProductCard({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return <article className={`group shrink-0 overflow-hidden rounded-lg border border-border bg-card shadow-card transition duration-300 hover:-translate-y-1.5 hover:border-primary/55 hover:shadow-card-hover ${compact ? "w-[76vw] max-w-[270px] lg:w-auto lg:max-w-none" : ""}`}>
    <div className="relative overflow-hidden bg-muted">
       <img src={product.image} alt={product.imageAlt} width={400} height={400} loading="lazy" className="aspect-square w-full object-cover transition duration-500 group-hover:scale-[1.06]" />
      <div className="absolute left-3 top-3 flex gap-2">{discount > 0 && <Badge variant="offer">-{discount}%</Badge>}<Badge variant="outline" className="border-border/70 bg-background/90 text-[10px]">DEMO</Badge></div>
       <Button variant="surface" size="icon" className="absolute right-3 top-3 opacity-90 sm:translate-y-1 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100" aria-label={`Add ${product.name} to wishlist`}><Heart /></Button>
    </div>
    <div className="p-4">
      ...
      <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{product.description}</p>
       <div className="mt-3 flex flex-wrap items-baseline gap-2"><span className="font-display text-lg font-bold text-primary">{formatCedis(product.price)}</span>{product.previousPrice && <span className="text-xs text-muted-foreground line-through">{formatCedis(product.previousPrice)}</span>}</div>
